...
def main():
    ...
    parser.add_argument("--amp", action="store_true",
                        help="fp16 autocast on CUDA (faster on RTX A6000; OFF by default for the exact float32 protocol)")
    ...
    if device.type == "cuda":
        torch.backends.cudnn.benchmark = True     # fixed input size -> faster cuDNN
    print(f"device: {device}" + ("  [AMP on]" if config.amp else ""))
