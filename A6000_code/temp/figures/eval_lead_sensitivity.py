...
for i in range(B):
    ...
    out=m.sample_posterior(c,lead_b,y=yi,mask=mi,R=0.1,steps=25,guidance=1.0,ensemble=4,
...
print("\nLead-time sensitivity (45 held-out gauges, metres):")
for L in LEADS:
    print(f"  L={L:2d}  MAE = {np.mean(mae[L])*STD:.4f} m")
np.savez("temp/figures/lead_sensitivity.npz", leads=np.array(LEADS),
         mae_m=np.array([np.mean(mae[L])*STD for L in LEADS]))
print("saved lead_sensitivity.npz")
