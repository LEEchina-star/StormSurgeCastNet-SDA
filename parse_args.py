...
def create_parser(mode='train'):
    ...
    # 数据保存的地方
    parser.add_argument("--root", default=os.path.expanduser('Data2'), type=str, help="path to your copy of the dataset")
    ...
    # ===================== SDA-Diff (conditional diffusion + data assimilation) =====================
    # model architecture
    parser.add_argument("--diff_dim", default=64, type=int, help="base channel width of the conditional U-Net")
    parser.add_argument("--diff_levels", default=3, type=int, help="number of encoder/decoder levels")
    parser.add_argument("--diff_emb_dim", default=256, type=int, help="embedding dim for sigma/lead conditioning")
    parser.add_argument("--diff_attn_level", default=2, type=int, help="encoder level that gets self-attention")
    parser.add_argument("--diff_dropout", default=0.0, type=float, help="dropout in ResBlocks")
    parser.add_argument("--diff_in_ch", default=1, type=int, help="channels of the noisy target")
    parser.add_argument("--diff_out_ch", default=1, type=int, help="output channels of the denoiser (1 = sparse surge)")
    # EDM training schedule
    parser.add_argument("--sigma_data", default=1.2, type=float, help="EDM sigma_data (typical std of the target; ~1.2 for this benchmark)")
    parser.add_argument("--p_mean", default=-1.2, type=float, help="lognormal mean of sigma sampling")
    parser.add_argument("--p_std", default=1.2, type=float, help="lognormal std of sigma sampling")
    parser.add_argument("--sigma_max", default=1.0, type=float, help="max noise level at inference sampling")
    parser.add_argument("--sigma_min", default=0.002, type=float, help="min noise level at inference sampling")
    parser.add_argument("--ema_decay", default=0.999, type=float, help="EMA decay for the denoiser weights")
    # inference / SDA sampling
    parser.add_argument("--sample_steps", default=50, type=int, help="annealed Langevin steps at inference")
    parser.add_argument("--sda_guidance", default=1.0, type=float, help="weight of the SDA likelihood gradient")
    parser.add_argument("--obs_noise", default=0.1, type=float, help="observation noise std R (meters)")
    parser.add_argument("--ensemble", default=8, type=int, help="number of posterior samples")
    parser.add_argument("--surge_threshold", default=0.5, type=float, help="threshold h (m) for exceedance probability P(surge>h)")
    # data / cache
    parser.add_argument("--cache_dir", default='', type=str, help="directory of pre-built sample caches (default: <root>/cache)")
    parser.add_argument("--gtsm_years", default='1993-2018', type=str, help="GTSM year window used by the loader, e.g. '2010-2014' (speeds up data access)")
    parser.add_argument("--use_residual", action="store_true", help="diffuse the GTSM residual (surge - GTSM_target) instead of raw surge")
    return parser
